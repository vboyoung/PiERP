...
$(document).ready(function() {
    ...
    const prop = $.extend( jsAA.gProp.getWriteProp(), {
		...
		loadComplete : function(){
            var editOpts = $('#voList').getColProp('bdgtDiv').editoptions;
...
$('#btnSave').click(function() {
    ...
    var delRow = [];
    var list = $('#list').getGridParam( 'data' );
    ...
    pierp.ui.confirm( '저장하시겠습니까?' ).then(function(btn) {
        if(btn == 'yes') {
            list.forEach(function(rowData) {
                ...
                var param = Object.assign(rowData, $('#srchBox').getFormData());
                    ...
                    params : param,
                      callback: function(status, resData) {
                          if( status == 'success' && resData.status == 'success' ) {
                              var errorMsg = resData.objects.errorMsg;
...
$('#btnLk').click(function() {
	var chkIndex = $('#indcList').getCheckedIndex();
	...
	// 선택한 데이터의 arr배열 얻기
	var chkData = chkIndex.map(function( rowid ) {
		return $('#indcList').getCell( rowid, 'evalIndcCd' );
	});
	
	// 세트 값 세팅
	var otEvalIndcCdSet = chkData.join('|');
...
$('#btnRegPop1').click(function() {
    ...
    var isModified = $('#list3').getGridParam('data').some(function(rowData) { return rowData.stateFlag != null; });
    if( isModified ) {
...
$('#btnAdd').click(function() {
    ...
    var comuActyYy = $('#srchBox [name=comuActyYy]').val();
    var comuActyHalfyyCd = $('#srchBox [name=comuActyHalfyyCd]').val();
    var stateFlag = $('#dtlBox [name=stateFlag]').val();
    var param = {
            stateFlag : 'I',
...
$('#btnFileDtls').click(function() {
    ...
    var indcRow = $('#indcList').getGridParam( 'selrow' );
    if( !indcRow ) {
    ...
    // 파일첨부 docId 얻어오기
    var rsltStat = $('#rsltBox [name=otEvalPrgsStatCd]').val() == '04';
    var dtlsDoctId = $('#dtlBox [name=evalDtlsAttchDoctId]').val();
    var docId = rsltStat || dtlsDoctId ? dtlsDoctId : fn_getAttchDocId();
...
function cfnFilePopup(rowId) {
	var attchDoctId = $('#mstBox [name=attchDoctId]').val();
	var permDivCd = $('#mstBox [name=permDivCd]').getSelectData();
	...
	var docId = data.attchDoctId;
	var lvbcAplNo = data.lvbcAplNo;

	$('#attachFile [name=attchDoctId]').val(docId);
	$('#attachFile [name=lvbcAplNo]').val(lvbcAplNo);

	// 첨부파일 팝업 호출
	fileUploadPop = cfn_OpenFileAttach({
		programId : pId,
		docId : docId,
		readonly : true,
		multiple : false,
		...
		iDSs : [
		  ...
		  callback: function(status, resData) {
			  if( status == 'success' && resData.status == 'success' ) {
				  ...
				  var indcList = resData.objects.indcList.rows;
				  var chkedRow = indcList.filter(function(data) { return data.chk; });
...
function fn_changeDtlBox( chgObj ) {
	var boxid		= $(chgObj).closest('.indcDtlBox').attr('id');	// 수정한 boxid
	var cellname	= $(chgObj).attr('name');						// 수정한 cellname
	var divCd		= boxid.replace( 'DtlBox', '' );				// boxid의 구분값 추출 - mesu:계량 / nmesu:비계량
	var gridObj		= $('#'+divCd+'List');							// grid객체
...
function fn_gridOnSelectRow( gridDiv, rowid ) {
	...
	var formNm = gridDiv+'DtlBox';

	// 폼에 데이터 세팅
	var rowData = $('#'+gridDiv+'List').getRowData( rowid );
	$('#'+formNm).setFormData( rowData );

	// 폼 활성화
	var prgsStatCd = $('#indcBox [name=intrEvalPrgsStatCd]').val();
