...
function appendListRow(index, rowData){
    ...
    var delayBtn = '<span id="delay_btn" class="btn delay_btn" onclick="delayCancel(\'' + index +'\');">취소</span>';
	...
	$("#DataList > tbody").append(
        '<tr data-id="'+todoVO.todoId+'" data-seq="' + todoVO.seq + '" data-objt="' + todoVO.objt +'" data-stat="'+ todoVO.oldStatCd +'" data-impr="'+ todoVO.impr +'">' +
			'<td>' + chkBox + '</td>' +
			'<td>' + delayBtn + '</td>' +
			'<td>' + todoVO.tyCdNm + '</td>' +
			'<td>' + todoStat + '</td>' +
...
function delayCancel(idx){
    ...
    var oldStatCd = $('#DataList tbody').children('tr:eq(' + idx + ')').data('stat');
    ...
    param.seq = seq;
    param.objt = objt;
    param.oldStatCd = oldStatCd;
    param.todoDiv = 'D';

    console.log(param);

    var message = "선택하신 TO-DO의 보류 상태를 취소하시겠습니까?";
	       ...
	       <nav class="navbar search-form">
  			...
  			<div class="sort_right">
				<form role="search" name="searchForm" id="searchForm">
		           ...
		           <!--  파라미터 전송 -->
                    ...
                    <input type="hidden" name="todoDiv" value="D">
			...
			<div class="bottom_wrap">
				<nav class="page-area">
			        <div id="paging"></div>
			        <div class="btn_wrap">
<!-- 			            <span id="btn" class="btn">관리자</span> -->
			        </div>
			    </nav>
			</div>
